 ...
 * Joomla 5.x + PHP 8.1+
 *
 * Game: Illinois Extra Shot (IL4)
 *   game_id        = IL4
 *   main balls     = 6  (columns: first, second, third, fourth, fifth, sixth)
 *   main range     = 01..50
 *   Extra Shot col = seventh, range 01..25
 *
 * Upstream variables assumed present:
 *   $stateName, $stateAbrev, $gName, $dbCol
 */
 ...
 */
$gameId        = 'IL4';
$mainCols      = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth'];
$mainBallCount = count($mainCols);
$mainMin       = 1;
$mainMax       = 50;
$bonusCol      = 'seventh';
$bonusMin      = 1;
...
$seoStateName = isset($stateName) ? (string) $stateName : '';
$seoGameName  = isset($gName)     ? (string) $gName     : '';

$metaDesc = '';

if ($seoStateName !== '' && $seoGameName !== '') {
    $doc->setTitle('Results Intelligence — ' . $seoStateName . ' • ' . $seoGameName . ' | LottoExpert.net');
    $metaDesc = 'Analyze ' . $seoStateName . ' ' . $seoGameName
        . ' lottery numbers with verified draw data. View frequency, recency, hot and cold numbers,'
        . ' and draw history for the ' . $seoStateName . ' ' . $seoGameName
        . ' lottery at LottoExpert.net.';
...
if ($loginStatus === 1) {
    ...
    $rawPhone = (string) $db->loadResult();

    if ($rawPhone !== '') {
        $rawPhone  = str_replace('"', '', $rawPhone);
        $rawPhone  = str_replace('(', '', $rawPhone);
        $userPhone = str_replace(')', '-', $rawPhone);
    } else {
        $userPhone = 'NULL';
    }
}
...
[$bonusCounts, $bonusLastSeenIndex] = leInitRange($bonusMin, $bonusMax);
...
$bonusChartLabels = leBuildNaturalLabels($bonusMin, $bonusMax);
$bonusChartValues = [];
...
$repeatedNumbers   = leFindRepeatedFromLatest($latestMainBalls, array_slice($rowsMain, 1, 10), $mainCols);
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate(
            $db, (string) $dbCol, $gameId, $drawDate, $ball, false, $mainCols, $bonusCol
        );
    ...
    if ($hasBonus && $es !== '') {
        $prevEsDate = leGetPreviousOccurrenceDate(
            $db, (string) $dbCol, $gameId, $drawDate, $es, true, $mainCols, $bonusCol
        );
        $esDrawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevEsDate, $drawDate);

        $drawHistoryRows[] = [
            'label'    => $bonusLabel . ' (' . lePad2($es) . ')',
            'prevDate' => $prevEsDate,
            'drawsAgo' => $esDrawsAgo,
            'isBonus'  => true,
        ];
...
$mainMaxPadded  = str_pad((string) $mainMax,  2, '0', STR_PAD_LEFT);
$bonusMaxPadded = str_pad((string) $bonusMax, 2, '0', STR_PAD_LEFT);
...
$jsonLdData = [
    ...
    '@type'       => 'WebPage',
    'name'        => ($seoStateName !== '' && $seoGameName !== '')
        ? ($seoStateName . ' ' . $seoGameName . ' Results Analysis | LottoExpert.net')
        : 'Lottery Results Analysis | LottoExpert.net',
    'description' => $metaDesc,
    'url'         => $canonicalNoQuery,
];

$jsonLdOutput = json_encode($jsonLdData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

if ($jsonLdOutput !== false) {
...
.skai-chart-frame--tall{height:1040px}
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            ...
            <?php if ($hasBonus && $es !== '') : ?>
              ...
              <span class="skai-ball skai-ball--bonus" title="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($es), ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endif; ?>
          </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel below shows the complete main-number distribution across all values 01&ndash;<?php echo htmlspecialchars($mainMaxPadded, ENT_QUOTES, 'UTF-8'); ?>. The second panel shows the complete <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution for the selected bonus window.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo htmlspecialchars($mainMaxPadded, ENT_QUOTES, 'UTF-8'); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-wrap">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;<?php echo htmlspecialchars($bonusMaxPadded, ENT_QUOTES, 'UTF-8'); ?> across the last <?php echo (int) $nodCurrentBonus; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-wrap">
                <div class="skai-chart-frame">
                  <canvas id="bonusChart" aria-label="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo htmlspecialchars($mainMaxPadded, ENT_QUOTES, 'UTF-8'); ?></span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            ...
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo htmlspecialchars($bonusMaxPadded, ENT_QUOTES, 'UTF-8'); ?></span>
          </div>

          <div class="skai-table-wrap">
            <table id="skai-bonus-table" class="skai-table" aria-label="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = $bonusMin; $i <= $bonusMax; $i++) : ?>
                  <?php
                  $number      = ($i < 10) ? '0' . $i : (string) $i;
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of <?php echo htmlspecialchars((string) $stateName . ' ' . (string) $gName, ENT_QUOTES, 'UTF-8'); ?> easier to understand, compare, and carry into deeper SKAI analysis. The main number pool covers 01&ndash;<?php echo htmlspecialchars($mainMaxPadded, ENT_QUOTES, 'UTF-8'); ?> and the <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> pool covers 01&ndash;<?php echo htmlspecialchars($bonusMaxPadded, ENT_QUOTES, 'UTF-8'); ?>. All counts and recency figures respect this separation.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function safeCanvas(id) {
    ...
    if (!el) { return null; }
    if (el.parentNode && el.parentNode.offsetWidth === 0) { return null; }
    return el;
  }
